...
- **Word/Excel flat-form fill (RFC-0002 Phase B):** for  with **no** named regions, values are placed by detecting flat labels — Word table label→next cell and “Label:” paragraphs; Excel label cell→right/below neighbour (reads shared strings). Runs as a fallback after the named-region pass; on-device.
