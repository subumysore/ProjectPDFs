...
### Added — multi-step application fill (one press carries the whole wizard)
- `multistep.js` (`runStepLoop`, `summarise`) + `stepprobe.js` (`stepProbe`, injected and
  self-contained) + a `fillSteps` handler in the BACKGROUND service worker, so closing the popup can no
  longer abandon a half-filled application. Each step is a fresh `fillPage` injection — there is no
  second filling engine. Per docs/specs/multi-step-fill.md.
- The loop fills a step, BANKS that step's answers into the vault (key/value, honouring "save new
  details", never an image), then looks for a way forward: Next / Continue / Save and continue. It STOPS,
  and says why, when a required question is still unanswered ("wait, don't skip"), at a Submit control or
  a "Review your application" page, when nothing advances, when the step does not change, or after 12
  steps. **Submit is never pressed — that click is the user's, always.**
- Popup "Fill this form" drives it and reports one plain-language summary (fields filled, answers saved,
  and why it stopped). A form in another language still takes the existing single-page translating path.
- New saved answer: **"Able to work onsite / hybrid?"** — a hybrid-working question is not a relocation
  question, and was being left blank.
### Fixed — screening and self-ID questions on CUSTOM dropdowns were never answered
- Saved answers reached radio groups and native `<select>`s only, so on every modern ATS (Greenhouse,
  Workday, Lever) work authorisation, Gender, Hispanic/Latino, Race, Veteran and Disability status stayed
  empty even with the answers saved. A new pass answers them on custom widgets too — same contract: never
  guessed, only an answer the user already gave, only a row belonging to THAT widget, blank if absent.
  Answering one question re-renders the whole list, so the widgets are re-read between answers (that alone
  cost every question after the first). Verified live on Greenhouse.
- A widget's question is read from its `label[for]` as well, and a wrapper showing only "Select…" no
  longer passes as the question.
- **Country**: a row decorated with its dialling code ("United States +1") is the same country; a country
  list is not scored as a dialling list; "United States of America" and other official long names
  canonicalise to the same country; and a row we clicked that the library has not committed yet falls
  through to the per-technology adapters instead of being called "not in this list".
- **Vault keys**: a dropdown's option list and the EEO definition paragraphs were being saved as part of
  the KEY ("GenderSelect ...MaleFemale…"). The question now ends at the widget's placeholder, and a
  paragraph is never saved as a key.
- **The pause list**: a phone box holding "+1 919 555 0123" was reported as unanswered (a neighbouring
  country widget was read instead of the field itself), and hidden mirror inputs appeared as
...
### Added — website: "EXE vs Extension" comparison at the top of the landing page
- New `#compare` section, generated by `docs/marketing/build-site.mjs` (`VS_CSS` + `vsSection()`), sits
  immediately under the hero CTAs — a visitor sees what the **desktop app (.exe)** does versus what the
  **browser extension** does before scrolling. Side-by-side table over 9 capabilities (PDF fill, flat-PDF/
  scan→fillable, Word/Excel, web autofill, cross-language fill, signing, vault, offline, install), with
  ✅ / — per surface; collapses to stacked cards under 720px, RTL-aware.
- **Last row = the downloads**, as icon buttons: 🖥️ Download for Windows (`/dl/exe`) and 🧩 Add to Chrome
  (Web Store). Nav and hero CTAs now jump to `#compare`, so no scrolling is needed to download.
### Changed
- "Get it" retired as a label: nav CTA → **Download**, hero → **Download (beta)**, store button →
  **Add to Chrome — Web Store** (the download is free; the app itself prompts for a license).
- New `vs.*` strings in `docs/marketing/i18n/en.json`; the other 25 locales fall back to English for the
  new block until translated (existing translated copy is untouched — additive only).
