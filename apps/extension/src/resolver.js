...
export function resolveFields(vault, fields) {
  ...
  const ALIASES = {
    ...
    // — payment CARD + its BILLING address (from a saved card record; the record's own keys
    //   normalise into these synonyms so the value is picked up, and a payment form's captions
    //   score against the same lists). Billing is kept SEPARATE from the mailing address so a
    //   saved card never overwrites a normal address field. —
