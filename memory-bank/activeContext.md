...
- Header repair: nav must NOT flex-wrap (it orphaned the brand mark and shoved the counter off-screen); language switcher now lives inside the nav instead of its own full-width strip; counter chip shows the number only because nav is capped at 1080px by .wrap. Single 68px row at 1100-1920px, checked live. Install/SmartScreen caveat re-homed as fine print under the comparison table.
- Per-surface download counts (EXE / EXT) now render under each download button in the comparison table, fed by the same /counts fetch as the nav total chip. Live: 6,020 / 6,007 of 12,027.
